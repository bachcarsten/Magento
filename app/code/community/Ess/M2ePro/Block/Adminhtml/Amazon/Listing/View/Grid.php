...
// todo new asin
//        $this->getMassactionBlock()->addItem('new_asin', array(
    ...
    private function getGeneralIdColumnValueEmptyGeneralId($row)
    {
        ...
        if (!empty($tempCategoryId)) {
            ...
            $newAsinTemplateUrl = $this->getUrl(
                '*/adminhtml_amazon_category/edit/',
                array(
                    'id' => $tempCategoryId,
                    'marketplace_id' => $marketplaceId,
                    'listing_product_ids' => $lpId
                )
            );
            ...
            $iconSrc = $iconPath.'unassign.png';
                ...
                return <<<HTML
&nbsp;
<a href="{$newAsinTemplateUrl}" target="_blank">New ASIN</a>
<br>
<a href="javascript:;" title="{$tip}"
   onclick="AmazonListingProductSearchHandlerObj.showUnmapFromAsinPrompt('{$lpId}');">
...
HTML;
        ...
        $text .= <<<HTML
<a href="javascript:;" onclick="AmazonListingProductSearchHandlerObj.showUnmapFromAsinPrompt({$lpId});" title="{$tip}">
