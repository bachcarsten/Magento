...
class IllApps_Shipsync_Model_Shipping_Carrier_Fedex extends Mage_Usa_Model_Shipping_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface
{
	...
	{
	    /** Estimate packages */
	    if ($packages = $this->estimatePackages($r->getItems(), $r->getDefaultPackages()))
	    {
		/** Debug to firebug */
     ...
     * @return array
     */
    public function estimatePackages($items, $defaultPackages)
    {	
	/** Unset 'special packaging' */
	unset($defaultPackages[0]);
	...
	foreach ($items as $item)
	{
	    ...
	    {
		/** If no package has been started */
		if (!isset($packages[$i]['weight']))
		{
		    /** Iterate through default packages to find best fit */
		    foreach ($defaultPackages as $defaultPackage)
		    {
			/** If item fits */
			if ((isset($item['volume']) && ($item['volume'] <= $defaultPackage['volume'])) ||
			   (!isset($item['volume']) && ($item['weight'] <= $defaultPackage['weight'])))
			{
			    ...
			    $packages[$i]['weight']     = $item['weight'];	     /** Set package initial weight */
			    ...
			    /** Set package volume */
			    if (isset($item['volume'])) { $packages[$i]['volume'] = $item['volume']; }

			    /** Package is started, and item is added, so break out of this loop */
			    break;
			}
		    }
		}
		/** Else, if package has been started */
		else
		{
		    /** Check if item fits */
		    if ((isset($item['volume']) && isset($packages[$i]['volume'])			   /** If volume is set */
			   && ($item['volume'] + $packages[$i]['volume']) <= $packages[$i]['max_volume'])  /** Compare volumes */
		    || (!isset($item['volume'])								   /** Or, if weight is set */
			   && ($item['weight'] + $packages[$i]['weight']) <= $packages[$i]['max_weight'])) /** Compare weights */
		    {
			$packages[$i]['items'][]  = $item;           /** Add item to package */
			$packages[$i]['weight']  += $item['weight']; /** Add item's weight to package */

			/** If item has volume, add this to package volume */
			if (isset($item['volume'])) { $packages[$i]['volume'] += $item['volume']; }
		    }
		    /** If item doesn't fit, move to a new package */
		    else 
		    {
			$i++;
	...
	if (isset($special_packages) && is_array($special_packages))
	{
	    ...
	    foreach ($special_packages as $package) { $packages[] = $package; }
	}
	...
	foreach ($rateReplyDetails as $rateReply)
	{
		...
		/** If show_timestamp is enabled && delivery timestamp is available */
		if ($this->getConfigData('show_timestamp') && isset($rateReply->DeliveryTimestamp))
		{
		...
		/** If disable VAT */
		if ($this->getConfigData('disable_vat'))
		{
		    /** If multiple surcharges */
		    if (is_array($shipmentRateDetail->Surcharges))
		    {
			/** Iterate through surcharges */
			foreach ($shipmentRateDetail->Surcharges as $surcharge)
			{
			    /** If surcharge is VAT */
			    if ($surcharge->SurchargeType == 'VAT')
			    {
				/** Deduct from rate */
				$rate = $rate - $surcharge->Amount->Amount;
			    }
			}
		    }
		    /** Else, if only one surcharge and it is VAT */
		    elseif ($shipmentRateDetail->Surcharges->SurchargeType == 'VAT')
		    {
			/** Deduct from rate */
			$rate = $rate - $shipmentRateDetail->Surcharges->Amount->Amount;
		    }
		}
	...
	/** Iterate through each package to ship */
		...
		/** Get tracking object */
                $track = Mage::getModel('sales/order_shipment_track')
                    ->setTitle($r->getMethodCode())					/** Set method */
		...
		/** Save transaction */
                ...
                /** Send shipment email TODO: add option for this */
		$shipment->sendEmail();
    ...
    protected function _parseWsdlTrackingResponse($trackingvalue, $response)
    {
        ...
        if ($response->HighestSeverity != 'FAILURE' && $response->HighestSeverity != 'ERROR')
        {
            ...
            if (isset($response->TrackDetails->EstimatedDeliveryTimestamp))
                { $resultArr['deliverydate'] = $response->TrackDetails->EstimatedDeliveryTimestamp; }
