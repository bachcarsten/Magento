    ...
    public function collect(Mage_Sales_Model_Quote_Address $address) {
		...
		if($address->getPostcode() && $address->getPostcode() !='-'){
			...
			if(Mage::helper('avatax')->isAddressActionable($address->getQuote()->getShippingAddress(), $address->getQuote()->getStoreId())){ //Added check for calculating tax for regions filtered in the admin
                ...
                // Goodahead: add surcharge tax
                if($address->getFoomanSurchargeAmount()) {
                    $surchargeItem = new Varien_Object();
                    $surchargeItem->setId('surcharge')
                        ->setAddress($address);
                    $baseSurchargeTax = $calculator->getItemTax($surchargeItem);
                    $surchargeTax = Mage::app()->getStore()->convertPrice($baseSurchargeTax);

                    $address->setBaseFoomanSurchargeTaxAmount($baseSurchargeTax);
                    $address->setFoomanSurchargeTaxAmount($surchargeTax);

                    $this->_addAmount($surchargeTax);
                    $this->_addBaseAmount($baseSurchargeTax);
                }
			}
		}
