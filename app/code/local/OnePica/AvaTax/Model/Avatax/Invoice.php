...
class OnePica_AvaTax_Model_Avatax_Invoice extends OnePica_AvaTax_Model_Avatax_Abstract
{
	...
	public function invoice($invoice, $queue) {
        ...
        // Add surcharge (Fooman Surcharge extension)
        $this->_addSurcharge($invoice);
	...
	*/
     ...
     * @return int
     */
    protected function _addSurcharge($invoice)
    {
        $invoiceCollection = Mage::getModel('sales/order_invoice')->getCollection()
            ->addAttributeToFilter('order_id', $invoice->getOrderId());
        if ($invoiceCollection->getSize() > 1) {
            return false;
        }

        if (0 < $invoice->getOrder()->getFoomanSurchargeAmount()) {
            $lineNumber = count($this->_lines);
            $surchargeAmount = $invoice->getOrder()->getFoomanSurchargeAmount();
            $baseSurchargeAmount = $invoice->getOrder()->getBaseFoomanSurchargeAmount();

            $line = new Line();
            $line->setNo($lineNumber);
            $line->setItemCode('surcharge');
            $line->setDescription('Surcharge amount');
            $line->setQty(1);
            $line->setAmount($surchargeAmount);
            $line->setDiscounted(false);

            $this->_lineToItemId[$lineNumber] = 'surcharge';
            $this->_request->setLines($this->_lines);
            $this->_lines[$lineNumber] = $line;
            return $lineNumber;
        }
