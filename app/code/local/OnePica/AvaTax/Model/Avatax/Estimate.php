...
class OnePica_AvaTax_Model_Avatax_Estimate extends OnePica_AvaTax_Model_Avatax_Abstract {
     ...
     * @return int
     */
    protected function _addSurcharge($address) {
        if ($address->getFoomanSurchargeAmount()) {
            $lineNumber = count($this->_lines);
            $storeId = Mage::app()->getStore()->getId();
            $surchargeAmount = $address->getFoomanSurchargeAmount();
            $baseSurchargeAmount = $address->getBaseFoomanSurchargeAmount();
            ...
            $this->_lines[$lineNumber] = $line;
            $this->_request->setLines($this->_lines);
            $this->_lineToLineId[$lineNumber] = 'surcharge';
            return $lineNumber;
        }
    }
