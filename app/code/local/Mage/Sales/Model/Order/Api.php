...
class Mage_Sales_Model_Order_Api extends Mage_Sales_Model_Api_Resource
{
    ...
    public function info($orderIncrementId)
    {
        ...
        if(array_key_exists('authorize_cards', $order->getPayment()->getAdditionalInformation())){
            foreach($order->getPayment()->getAdditionalInformation('authorize_cards') as $key => $value){
            ...
            if (isset($aType[$ccType])) {
                ...
                $result['payment']['method'] = $ccTypeName;
            }
            else {
                $ccTypeName = Mage::helper('payment')->__('N/A');
            }
        }
        if($result['payment']['method'] == 'goodahead_authorizenet'){
            //$ccType = $result['payment']['cc_type'];
            //Mage::log(print_r($order->getPayment(), true), null, 'CIM.log', true);
        ...
        //$result['loworderfee'] = 10;
        //$result['base_loworderfee'] = $order->getBaseLoworderfee();
        //$result['lof_tax_amount'] = $order->getLofTaxAmount();
        //$result['base_lof_tax_amount'] = $order->getBaseLofTaxAmount();
