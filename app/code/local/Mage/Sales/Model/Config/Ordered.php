...
abstract class Mage_Sales_Model_Config_Ordered extends Mage_Core_Model_Config_Base
{
     ...
     * @var array
     */
    protected $_sortedCollectors = array();
...
//            uasort($configArray, array($this, '_compareTotals'));
            if ($this->_sortCollectors(array_keys($configArray), $configArray, 0)) {
                $sortedCollectors = $this->_sortedCollectors;
            } else {
                uasort($configArray, array($this, '_compareTotals'));
                $sortedCollectors = array_keys($configArray);
            }
...
//        $sortedCollectors = array_keys($configArray);
    ...
    protected function _sortCollectors($collectors, $collectorsData, $index)
    {
        $max = count ($collectors);
        $this->_sortedCollectors = array();
        for ($i = 0; $i < $max; $i++) {
            $skipCollectors = array_reverse(array_intersect($this->_sortedCollectors, $collectorsData[$collectors[$i]]['after']));
            if (count($skipCollectors)) {
                $index = array_search(array_shift($skipCollectors), $this->_sortedCollectors, true);
                array_splice($this->_sortedCollectors, $index+1, 0, array($collectors[$i]));
            } else {
                $this->_sortedCollectors[] = $collectors[$i];
            }
        }
        $result = $this->_validateCollectors($collectorsData);
        return $result;
    }

    protected function _validateCollectors($collectorsData)
    {
        $keysCollected = array();
        foreach ($this->_sortedCollectors as $collector) {
            if (
                is_array($collectorsData[$collector]['after'])
                && array_intersect($collectorsData[$collector]['after'], $keysCollected)
                != $collectorsData[$collector]['after']
            ) {
                return false;
            }
            if (
                is_array($collectorsData[$collector]['before'])
                && count(array_intersect($collectorsData[$collector]['before'], $keysCollected))
            ) {
                return false;
            }
            $keysCollected[] = $collector;
        }
        return true;
    }
