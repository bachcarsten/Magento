...
(function(jQuery) {
	...
	jQuery.fn.easySlider = function(options){
		...
		this.each(function() {  
			var obj = jQuery(this); 				
			var s = jQuery(".slideritem", obj).length;
			var w = jQuery(".slideritem", obj).width();
			var h = jQuery(".slideritem", obj).height();
			obj.width(w); 
			obj.height(h); 
			obj.css("overflow","hidden");
			var ts = s-1;
			var t = 0;
			jQuery(".slidercontent", obj).css('width',s*w);			
			if(!options.vertical) jQuery(".slideritem", obj).css('float','left');
			...
			function animate(dir,clicked){
				...
				if(!options.vertical) {
					p = (t*w*-1);
					jQuery(".slidercontent",obj).animate(
						{ marginLeft: p }, 
						speed
					);				
				} else {
